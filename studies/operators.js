 ...
 * a meaningful way. Operators are used to assign values to variables,
 ...
 */
 ...
 /**
 * Assignment operators assign values to variables based on what the operand is
...
/**
 * Arithmetic operators are binary operators that are simply used to perform 
...
/**
 * Comparison operators are binary operators that compares two operands and coerces
...
/**
 * Logical operators are binary operators that, similar to comparison operators,
 * are mainly used to control flow of a program. They do this by having a few different
 * conditions to be checked before running the code can be ran. Bang operators are a type 
 * of logical operator that checks if a value is truthy or falsy. Values with a boolean 
 * value of true are truthy, and values with a boolean value of false are falsy. 
 */
 
  let numOne = 10;
  let numTwo = 5;
  
  console.log(numOne > 10 && numTwo <= 5); // prints => false
  console.log(numOne + numTwo > 14 || numOne + numTwo > 16); // prints => true
...
/**
 * Unary operators only take in one operand. Some unary operators include assignment
 ...
 */
   ...
   // typeof returns a string indicating the type of the operand being passed
   // increment unary operators (++) will add one to its operand and then return that value
   // decrement unary operators (--) will subtract one from its operand, and then return that value
   // double bang unary operators (!!) will return an operands truthy value.
   
   // 6. String operators // 
   
   
/**
 * String operators are binary operators that help programmers manipulate strings.
...
/**   
 * Conditional operators are the only operators that are ternary operators because
 * they take in three operands. Conditional operators assigns a value to a variable
 * based on if a condition passes or not. It is essentially a short hand for a 
 * normal conditional statement with if-else statements. The first part of a ternary 
 * operator is the condition that you want to test, this result has to coerce to a boolean
 * value. Following the conditonal statement, you then place a question mark (?) followed by
 * a colon (:). If the conditional statement evaluates to true, the value between the question 
 * mark will and the colon be returned. Finally, if the conditional statement evluates to 
 * false, the value after the colon will be returned. 
 */
    ...
    /** NOTE: On line 159, anything inside of the parentheses is the conditional
